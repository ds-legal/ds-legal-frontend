...
const TaskModal = ({ showModal, task }) => {
  ...
  const { deleteTask, getAllTask } = useTask();
      ...
      <div className="bg-white p-4 sm:p-6 w-full max-w-md sm:max-w-xl lg:max-w-3xl rounded-2xl shadow-lg overflow-y-auto max-h-[90vh]">
        ...
        <div className='mt-6 flex flex-col sm:flex-row justify-between gap-4'>
          <button 
            ...
            className='bg-[#EB2525] text-white px-6 py-3 rounded-full w-full sm:w-auto'>
            Delete
          </button>
          <button 
            onClick={handleEdit}
            className='bg-[#1983D5] text-white px-6 py-3 rounded-full w-full sm:w-auto'>
            Edit
          </button>
        </div>
      </div>
